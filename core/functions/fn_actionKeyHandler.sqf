...
if(_curTarget isKindOf "House_F" && {player distance _curTarget < 12} && ((__GETC__(life_donator) != 0)) OR ((nearestObject [[16019.5,16952.9,0],"Land_Dome_Big_F"]) == _curTarget OR (nearestObject [[16019.5,16952.9,0],"Land_Research_house_V1_F"]) == _curTarget) ) exitWith {
...
if(isPlayer _curTarget && _curTarget isKindOf "Man") then {
	...
	if(((_curTarget getVariable["restrained",false]) || (_curTarget getVariable["civrestrained",false]))  && !dialog) then {
		...
		else
		{
            ...
            if((player getVariable["FAR_isUnconscious",false]) == 0) then {

...
if(!(_house in life_vehicles)) exitWith {hint "Tu dois être a l'intérieur de t'as maisons."};
_containers = _house getVariable["containers",[]];
_houseCfg = [(typeOf _house)] call life_fnc_houseConfig;
if(count _houseCfg == 0) exitWith {}; //What the fuck happened?
if(count _containers >= (_houseCfg select 1)) exitWith {hint "Tu ne peux plus placer de coutainers."};

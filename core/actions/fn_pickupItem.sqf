...
if((time - life_action_delay) < 2) exitWith {hint "Calme toi ! Pas aussi vite"};
...
{
	titleText[format["%1 a été retenu comme preuve, tu as reçue $%2 comme récompense.",_itemName,[(life_illegal_items select _illegal) select 1] call life_fnc_numberText],"PLAIN"];
...
if(_diff <= 0) exitWith {hint "Impossible de ramasser l'item, ton inventaire est remplis."};

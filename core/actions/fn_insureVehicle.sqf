...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")) then
{
	...
	if( ([(getPlayerUID player),(_vehicle getVariable["vehicle_info_owners", []])] call fnc_index) == -1) exitWith {hint "Ce véhicule ne vous appartient pas, vous ne pouvez pas l'assurer !";};

	/*_vInfo = _vehicle getVariable["dbInfo",[]];
	if(count _vInfo == 0) exitWith {hint "Votre véhicule n'est pas permanent, vous ne pouvez donc pas l'assurer.";};*/

	_price = ([(typeOf _vehicle),__GETC__(life_price_insurance)] call fnc_index);
	if(_price == -1) then {
		_price = 500000;
	} else {

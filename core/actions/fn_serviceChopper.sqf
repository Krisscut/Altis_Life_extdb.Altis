...
if(count _search == 0) exitWith {hint "Il n'y a pas d'hélicopters sur l'héliport'"};
if(life_liquide < 1000) exitWith {hint "Tu as besoin de $1,000 pour réparer un hélicopter"};
...
_pgText ctrlSetText format["Réparation de l'hélicopter (1%1)...","%"];
...
while {true} do
{
	...
	_pgText ctrlSetText format["Réparation de l'hélicopter (%1%2)...",round(_cP * 100),"%"];
	if(_cP >= 1) exitWith {};
};

if(!alive (_search select 0) || (_search select 0) distance air_sp > 10) exitWith {life_action_inUse = false; hint "Le véhicule n'est plus en vie ou n'est plus sur l'héliport!"};

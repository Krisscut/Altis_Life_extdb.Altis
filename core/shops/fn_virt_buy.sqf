...
if((_price * _amount) > life_liquide && {!isNil "_hideout" && {!isNil {grpPlayer getVariable "gang_bank"}} && {(grpPlayer getVariable "gang_bank") <= _price * _amount}}) exitWith {hint "You don't have that much money!"};
if((_price * _amount) > life_liquide) exitWith {[false,_type,_amount] call life_fnc_handleInv; hint "You don't have that much money!";};

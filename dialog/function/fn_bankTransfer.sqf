...
if(_val > life_dabliquide) exitWith {hint "You don't have that much in your bank account!"};
_tax = [_val] call life_fnc_taxRate;
if((_val + _tax) > life_dabliquide) exitWith {hint format["You do not have enough money in your bank account, to transfer $%1 you will need $%2 as a tax fee.",_val,_tax]};
